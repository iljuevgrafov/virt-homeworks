...
Версия Linux 2.6 и старше позволяет сконфигурировать поведение ядра, чтобы ограничить "перевыделение" памяти. Данная настройка не  
...
в скрипт запуска постгреса, перед строкой где выполняется запуск процесса postmaster. 
