...
Версия Linux 2.6 и старше позволяют модифицировать конфигурацию ядра, чтобы ограничить "перевыделение" памяти. Данная настройка не  
исключит вызов OOM Killer, но значительно сократит вероятность его вызова.  Настройка можно выполнить командой sysctl -w vm.overcommit_memory=2,  
...
что процесс постгресса не будет терминирован компонентом OOM-Killer. Для этого нужно доабвить строку echo -1000 > /proc/self/oom_score_adj  
