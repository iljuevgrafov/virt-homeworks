...
Если redis работает в режиме активной отчистки истекших ключей, он производит очистку каждые 100 милисекунд, и повторяет операцию, если более 25% от кол-во ключей истекли.

Если в базе истекает определенное кол-во ключей одномоментно, и это кол-во более 25% от объема уже истекших ключей, redis блокирует базу чтобы запустить процесс очистки, который понизит кол-во истекших ключей ниже 25%.
...
Так же проблема может быть в том, что при появлении новой реплики был превышен общей лимит памяти для кластера.
 ```
## Задача 3
...
```
В таком случае у меня только один вариант - что код запросов написан не эффективно, это вызывает нехватку памяти при выполнении. Других вариантов у меня нет.
```
---
